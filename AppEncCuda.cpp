    ...
    bool &bOutputInVidMem, int32_t &cuStreamType)
{
    ...
    for (i = 1; i < argc; i++)              //读取命令内的参数，进行相关操作
    {
		if (!_stricmp(argv[i], "-h"))
        {
            ShowHelpAndExit();
        }
        if (!_stricmp(argv[i], "-i"))     //-i xxx  设置输入为xxx
        {
            if (++i == argc)
            {
                ShowHelpAndExit("-i");
            }
            sprintf(szInputFileName, "%s", argv[i]);  //将xxx写入输入的buffer
            continue;
        }
        if (!_stricmp(argv[i], "-o"))     //-o xxx  设置xxx为输出
        {
            if (++i == argc)
            {
                ShowHelpAndExit("-o");
            }
            sprintf(szOutputFileName, "%s", argv[i]);     //将xxx写入输出的buffer
            continue;
        }
        if (!_stricmp(argv[i], "-s"))   //设置分辨率为 nWidthxnHeight
        {
...
void EncodeCuda(int nWidth, int nHeight, NV_ENC_BUFFER_FORMAT eFormat, NvEncoderInitParam encodeCLIOptions, CUcontext cuContext, char *szInFilePath, int encoder_num, char *szOutFilePath, char *cvReadPath)
...
{
	// Open input file      也可以不要，换用opencv读图; 直接将szInFilePath输给EncodeCuda()
	...
	// Open output file       同fpIn，只需要将szOutFilePath输给EncodeCuda()
	...
	//std::string cv_path = szOutFilePath;
	...
	std::cout << "nFrameSize is " << nFrameSize << std::endl;
	...
	while (true)
	{
		if (i < encoder_num)
		{
			...
			sprintf(pic_name, "%d", i);
			std::cout << "pic_name is " << pic_name << std::endl;

			//cv::Mat img = cv::imread(cvReadPath + std::string(pic_name));
			//cv::Mat img = cv::imread("D:\\xjwjj\\1106\\0.bmp");


			cv::Mat img = cv::imread(std::string(szInFilePath) + "\\" + std::string(pic_name) + ".bmp", 1);
			std::cout << "rows:" << img.rows << std::endl;

			//std::memcpy(pHostFrame.get(), img.datastart, 7680 * 4320 * 3 + 54 );
			std::memcpy(pHostFrame.get(), img.data, 7680 * 4320 * 3 + 54 );     //pHostFrame.get()传的是数组地址
			...
			/*if (nRead != nFrameSize)
			{
				std::cout << "nRead != nFrameSize" << std::endl;
				break;
			}*/
...
{
	char szInFilePath[256] = "D:\\xjwjj\\1106",
		 cvReadPath[256] = "D:/xjwjj/1106/",
         szOutFilePath[256] = "D:\\Video_Codec_SDK_11.0.10\\Video_Codec_SDK_11.0.10\\Samples\\output\\test_video003.mkv";
    int nWidth = 7680, nHeight = 4320;
	int i = 0;
	//sprintf(szInFilePath, "D:\\xjwjj\\1106\\%d.bmp", i);
    ...
    {		
			...
			//std::cout <<"iscodeHEVC" << encodeCLIOptions.IsCodecHEVC() << std::endl;

			//CheckInputFile(szInFilePath);  // 可以不要
			ValidateResolution(nWidth, nHeight);

			if (!*szOutFilePath)
			{
				sprintf(szOutFilePath, encodeCLIOptions.IsCodecH264() ? "out.hevc" : "out.h264");
			}
			...
			if (!fpOut)
			{
				...
				if (bOutputInVideoMem)
				{
					...
					std::cout << "Please Set Output in Cuda Memory!" << std::endl;
				}
				else
				{
					EncodeCuda(nWidth, nHeight, eFormat, encodeCLIOptions, cuContext, szInFilePath, encoder_num,  szOutFilePath, cvReadPath);
					//int nWidth, int nHeight, NV_ENC_BUFFER_FORMAT eFormat, NvEncoderInitParam encodeCLIOptions, CUcontext cuContext, char *szInFilePath, int encoder_num, char *szOutFilePath
				}
