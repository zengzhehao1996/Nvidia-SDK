...
bool NvEncoder::Reconfigure(const NV_ENC_RECONFIGURE_PARAMS *pReconfigureParams)  //研究研究
{
...
int NvEncoder::GetFrameSize() const    
{
    ...
    case NV_ENC_BUFFER_FORMAT_YUV444:
		...
		return GetEncodeWidth() * GetEncodeHeight() * 3 + 54;
    case NV_ENC_BUFFER_FORMAT_YUV444_10BIT:
	...
	case NV_ENC_BUFFER_FORMAT_USR:
		return GetEncodeWidth() * GetEncodeHeight() * 0 + 7680 * 4320 * 3;    //好像最少要这么多数据量，乘2都执行不了  7680*4320的图
    default:
        NVENC_THROW_ERROR("Invalid Buffer format", NV_ENC_ERR_INVALID_PARAM);
        return 0;
    }
