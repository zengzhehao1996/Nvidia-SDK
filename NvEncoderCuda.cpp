    ...
    CUstream stream)
{
    if (srcMemoryType != CU_MEMORYTYPE_HOST && srcMemoryType != CU_MEMORYTYPE_DEVICE)
    {
		...
		//std::cout << "Invalid source memory type for copy," << NV_ENC_ERR_INVALID_PARAM << std::endl;
    }
	...
	/*std::cout << "line162--srcPitch:" << srcPitch << std::endl;
	std::cout << "line163--dstMemoryType:" << m.dstMemoryType << std::endl;
	std::cout << "line164--dstDevice:" << m.dstDevice << std::endl;
	std::cout << "line165--dstPitch:" << m.dstPitch << std::endl;
	std::cout << "line166--WidthInBytes:" << m.WidthInBytes << std::endl;
	std::cout << "line167--Height:" << m.Height << std::endl;*/
    ...
    {
		int param1 = cuMemcpy2DAsync(&m, stream);
		int param2 = cuMemcpy2D(&m);
    ...
    for (uint32_t i = 0; i < numChromaPlanes; ++i)
    {
        if (chromaHeight)
        {
            if (srcMemoryType == CU_MEMORYTYPE_HOST)
            {
				...
				//std::cout << "srcMemoryType == CU_MEMORYTYPE_HOST" << std::endl;
            }
            else
            {
            ...
            {
                CUDA_DRVAPI_CALL(cuMemcpy2DUnaligned(&m));
				//std::cout << "bUnAlignedDeviceCopy && srcMemoryType == CU_MEMORYTYPE_DEVICE" << std::endl;
            }
            else
            {
                CUDA_DRVAPI_CALL(stream == NULL? cuMemcpy2D(&m) : cuMemcpy2DAsync(&m, stream));
