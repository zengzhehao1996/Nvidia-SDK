        ...
        for (unsigned i = 0; i < tokens.size(); i++)
        {
            ...
            if (tokens[i] == "-444")
            {
                if (IsCodecH264()) 
                {
					...
					std::cout << "NvEncoderCLIOptions.h - line275 - is H264" << std::endl;
                } else 
                {
                    config.encodeCodecConfig.hevcConfig.chromaFormatIDC = 3;
					std::cout << "NvEncoderCLIOptions.h - line275 - is HEVC" << std::endl;
                }
                continue;
            }
...
private:
    ...
    std::vector<std::string> tokens;
    GUID guidCodec = NV_ENC_CODEC_HEVC_GUID;     //改变编码方式为HEVC或H264
